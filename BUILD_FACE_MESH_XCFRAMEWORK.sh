...
arm_zip=$(find bazel-out/ -iname "FaceMeshIOSLibFramework.zip" | grep arm)
./mediapipe/examples/ios/facemeshioslib/patch_ios_framework.sh $arm_zip FaceMeshIOSLib.h
...
arm_framework=$(find bazel-out/ -iname "FaceMeshIOSLibFramework.framework" | grep arm)
cp -a $arm_framework ./frameworkbuild/FaceMeshIOSLibFramework/arm64
...
x86_zip=$(find bazel-out/ -iname "FaceMeshIOSLibFramework.zip" | grep x86_64)
./mediapipe/examples/ios/facemeshioslib/patch_ios_framework.sh $x86_zip FaceMeshIOSLib.h

# copy the patched framework to our folder
x86_framework=$(find bazel-out/ -iname "FaceMeshIOSLibFramework.framework" | grep x86_64)
cp -a $x86_framework ./frameworkbuild/FaceMeshIOSLibFramework/x86_64
